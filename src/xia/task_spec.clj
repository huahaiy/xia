(ns xia.task-spec
  ...
  (:require [charred.api :as json]
            ...
            [xia.task-spec.validate :as task-spec-validate]
            [xia.tool :as tool]))

(def ^:private task-spec-version task-spec-validate/task-spec-version)
...
(def ^:private expression-arities task-spec-validate/expression-arities)
(def ^:private planner-expression-operators task-spec-validate/planner-expression-operators)
...
(defn- nonblank-string
  [value]
  (task-spec-validate/nonblank-string value))

(defn- normalize-id
  [field value]
  (task-spec-validate/normalize-id field value))

(defn- normalize-kind
  [value]
  (task-spec-validate/normalize-kind value))
...
(defn- validation-error
  [message data]
  (task-spec-validate/validation-error message data))

(defn normalize-spec
  "Normalize and validate a declarative task spec."
  [spec]
  (task-spec-validate/normalize-spec spec))
...
(defn validate-spec
  ...
  [spec]
  (task-spec-validate/validate-spec
   spec
   {:registered-executors (registered-executors)
    :tool-exists? (fn [tool-id]
                    (some? (db/get-tool tool-id)))}))
