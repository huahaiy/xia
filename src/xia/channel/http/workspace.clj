...
(defn- scratch-pad->body
  ...
  {:id         (:id pad)
   ...
   :mime       (:mime pad)
   :version    (:version pad)
   :created_at (http-common/instant->str deps (:created-at pad))
   :updated_at (http-common/instant->str deps (:updated-at pad))})
...
(defn- local-doc->body
  ...
  {:id         (some-> (:id doc) str)
   ...
   :created_at (http-common/instant->str deps (:created-at doc))
   :updated_at (http-common/instant->str deps (:updated-at doc))})
...
(defn- artifact->body
  ...
  {:id         (some-> (:id artifact) str)
   ...
   :created_at (http-common/instant->str deps (:created-at artifact))
   :updated_at (http-common/instant->str deps (:updated-at artifact))})
              ...
              (let [{:keys [type]} (ex-data e)]
                (case type
                  ...
                  (http-common/json-response deps 409
                                             {:error "scratch pad version conflict"
                                              :details (select-keys (ex-data e)
                                                                    [:expected-version :actual-version])})
                  :scratch/not-found
                  (http-common/json-response deps 404 {:error "scratch pad not found"})
                  (http-common/json-response deps 400 {:error (.getMessage e)}))))))))))
...
(defn- local-doc-part-bytes
  ...
  (let [tempfile (or (:tempfile part) (get part "tempfile"))
    ...
    (try
      (http-common/read-body-bytes deps body)
      (finally
        (when tempfile
...
(defn- local-doc-upload-entries
  [deps req]
  (if (http-common/multipart-form-request? deps req)
    (multipart-local-doc-upload-entries deps req)
    (json-local-doc-upload-entries deps (or (http-common/read-body deps req) {}))))

(defn- local-doc-error->body
  [deps upload throwable]
  {:name  (get upload :name)
   :error (http-common/throwable-message deps throwable)
...
(defn- artifact-create-spec
  ...
  (let [payload (if (map? (get data "artifact"))
                  (get data "artifact")
                  data)]
    (cond-> {:source (or (some-> (get payload "source") (http-common/nonblank-str deps) keyword)
                         :manual)}
...
(defn handle-list-workspace-items
  ...
  (try
    (let [params       (http-common/parse-query-string deps (:query-string req))
          workspace-id (http-common/nonblank-str deps (get params "workspace_id"))
          top          (http-common/parse-optional-positive-long deps (get params "top") "top")
    ...
    (let [params       (http-common/parse-query-string deps (:query-string req))
      ...
      (if-let [item (workspace/get-item item-id :workspace-id workspace-id)]
        (let [payload-file (some-> (:payload-path item) io/file)
          ...
          (if bytes
            (http-common/download-response deps (:name item) (:media-type item) bytes)
            (http-common/json-response deps 404 {:error "shared workspace item not found"})))
        (http-common/json-response deps 404 {:error "shared workspace item not found"})))
    (catch clojure.lang.ExceptionInfo e
      (http-common/exception-response deps e))))
